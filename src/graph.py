...
def understand_node(state):
    state["understood"] = understand_query(state["question"])
    return state
...
def retrieve_node(state, collection, bm25, chunks):
    ...
    # embed_model is no longer passed — Chroma handles embedding via its
    # built-in SentenceTransformerEmbeddingFunction.
    def retrieve_fn(q, expected_version=None, **kw):
        return hybrid_retrieve(
            q, collection, bm25, chunks,
    ...
    if state["understood"]["intent"] == "migration":
        result = decompose_and_retrieve(
            routing["from_version"], routing["to_version"],
            lambda q, expected_version: retrieve_and_grade(
                q, expected_version, "migration", retrieve_fn
            ),
        )
        all_chunks = [c for hop in result.get("hops", []) for c in hop["chunks"]]
        state["chunks"] = all_chunks
        state["status"] = result["status"]
    else:
        result = retrieve_and_grade(
            state["question"], routing.get("version"),
            routing["doc_types"][0], retrieve_fn,
        )
...
def generate_node(state):
    gen = generate_answer(state["question"], state["chunks"])
    verdict = verify_answer(gen["answer"], state["chunks"])
...
def build_graph(collection, bm25, chunks, embed_model=None):
    ...
    graph.add_node("retrieve", lambda s: retrieve_node(s, collection, bm25, chunks))
    ...
    graph.add_conditional_edges(
        "route", lambda s: s["status"],
        {"ready": "retrieve", "needs_clarification": END, "not_found": END},
    )
    graph.add_conditional_edges(
        "retrieve", lambda s: s["status"],
        {"found": "generate", "not_found": END},
    )
