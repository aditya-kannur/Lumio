...
One Gemini call grades all retrieved chunks for the current attempt.
Maximum grading attempts = MAX_RETRIES + 1.
"""
...
You are grading retrieved document chunks for a developer documentation assistant.

For every candidate, check:

1. relevant:
   Does the chunk actually address the candidate question?

2. version_correct:
   Does the chunk match the expected API version?

3. doc_type_correct:
   Does the chunk match the expected document type?
...
{{
  ...
  "rewritten_questions": [
    {{
      "candidate_id": 0,
      "question": "clearer search question"
    }}
  ]
}}
...
- Only include a rewritten question for candidates that do not pass.
- Do not invent versions or facts.
"""
...
def _retrieve_groups(groups, retrieve_fn):
    """
    Retrieve top candidates for every group.

    retrieve_fn may return either:
      - a list of chunks
      - {"chunks": [...], "status": ...}

    This handles both forms safely.
    """
    ...
    for group in groups:
        ...
        chunks = chunks[:MAX_GRADING_CHUNKS]
...
def retrieve_and_grade_batch(groups, retrieve_fn):
    ...
    All groups/chunks are graded in a single Gemini call per attempt.
    """
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        # One Gemini call grades ALL candidates.
        ...
        for group_index, group in enumerate(current_groups):
            ...
            for chunk in group["chunks"]:
                ...
                if candidate_id in rewrites:
                    rewrite_by_group[group_index] = rewrites[
                        candidate_id
                    ]
        ...
        # Every group needs at least one passing chunk.
        if (
            len(passed_groups) == len(current_groups)
            and current_groups
        ):
            return {
                "status": "found",
        ...
        # Rewrite failed queries for the next retrieval attempt.
        current_groups = [
            {
                **group,
...
def retrieve_and_grade(
    ...
    """
    Backward-compatible wrapper for normal reference/diagnostic queries.
    """
